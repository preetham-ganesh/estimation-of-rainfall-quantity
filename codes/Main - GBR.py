...
for i in sams:
    ...
    for tr_i, ts_i in rkf.split(data):
        ...
        model = GradientBoostingRegressor(n_estimators=50, max_depth=5, min_samples_leaf=2, min_samples_split=i)
...
d['EVS'] = evs_f
d['R2'] = r2_f
df = pd.DataFrame(d, columns=['Min Samples Split', 'MSE', 'RMSE', 'MAE', 'MDAE', 'EVS', 'R2'])
print(df)
df.to_csv('C:\\Users\\Preetham G\\Documents\\Research Projects\\Ensemble Rainfall\\Results\\Main - GBR SAMS.csv', index=False)